...
import { calculateSpread, calculateTotalProfit } from '../utils/silverLogic';

export default function SpreadRisk() {
  const [activeTab, setActiveTab] = useState('spread'); // 'spread' hoặc 'portfolio'
  ...
  const handleCalculateSpread = () => {
    const res = calculateSpread(Number(bidPrice), Number(askPrice));
    setResult(res);
  };

  const handleAddTransaction = () => {
    setTransactions([...transactions, { buy_price: 12000000, sell_price: 12500000, quantity: 1 }]);
  };

  const handleUpdateTransaction = (index, field, value) => {
    const newTx = [...transactions];
    newTx[index][field] = Number(value);
    setTransactions(newTx);
  };

  const handleRemoveTransaction = (index) => {
    const newTx = transactions.filter((_, i) => i !== index);
    setTransactions(newTx);
  };

  const computedStats = calculateTotalProfit(transactions);
  const computedCapital = transactions.reduce((acc, tx) => acc + (tx.buy_price * tx.quantity), 0);

  return (
    <div>
      <div className="section-header" style={{ marginBottom: '16px' }}>
        <span className="section-title">Phân tích Rủi ro & Danh mục</span>
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
        <button 
          ...
          className={activeTab === 'spread' ? '' : 'btn-outline'}
        ...
        <button 
          ...
          className={activeTab === 'portfolio' ? '' : 'btn-outline'}
          style={{ padding: '6px 16px', fontSize: '11px', minWidth: 'auto' }}
        >
          Lợi Nhuận Danh Mục
        </button>
      ...
      {activeTab === 'spread' ? (
        <>
          <p style={{ color: 'var(--muted)', fontSize: '11px', marginBottom: '24px', letterSpacing: '0.04em' }}>
            Đánh giá mức độ rủi ro dựa trên độ giãn của giá mua bán từ tiệm vàng/bạc.
          </p>

          <div className="input-group">
            <label>Giá cửa hàng MUA VÀO (VND)</label>
            <input 
              type="number" 
              value={bidPrice} 
          ...
          <div className="input-group">
            <label>Giá cửa hàng BÁN RA (VND)</label>
            <input 
              type="number" 
              value={askPrice} 
          ...
          <button onClick={handleCalculateSpread} style={{ marginTop: '8px' }}>Đánh Giá Rủi Ro</button>
                ...
                <table style={{ borderCollapse: 'collapse', color: 'var(--text)' }}>
                  <tbody>
                    <tr>
                      ...
                      <td style={{ fontWeight: '500' }}>{computedCapital.toLocaleString('vi-VN')} VND</td>
                    </tr>
                    <tr>
